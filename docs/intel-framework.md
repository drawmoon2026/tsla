...
**v0 实现状态（2026-07-31，`intel/scoring.py`，纯规则无 LLM）**：
身位/事实/时效三分已实现——身位分按渠道 tier 映射（T0/T1=1.0、T2=0.7、
...
`SOURCE_CLASS`）。**意外分未实现**（如实返回 None）：度量"与共识的偏离"需要
共识基线数据，待期权隐含波动或分析师预期数据源接入；总权重暂 = 身位×事实×时效
（partial 标注）。评分在查询层实时计算，不回写 events 表；仪表盘情报流每行
显示总权重（悬停看分解），渠道卡权重列双显"人工先验 + 当前平均四维分"。
自测 `python -m intel.scoring`（tier/type/age 单调性断言）。
