...
- [x] 概率分层成本后回测（`research/prob_layered_backtest.py`）：**AUC 0.602 未在成本后存活**——各层单笔均值全为负（-5.1→-3.4bp），但方向性单调有效（胜率 60.3%→64.2%、回撤 -57%→-27%）。根因是 +0.5%/-1% 不对称障碍打平需 70.7% 胜率。结论：模型有效、交易结构不成立
- [ ] 可选下一步：重新设计障碍结构（对称或 TP>SL）+ 重训重验——新假设，注意同数据反复挖掘风险；或更多年份数据扩展 regime
